...
// Heal a user
// ======================================================================
const healMe = (socket, clients, args) =>{
    if (socket.player != null && socket.player.body != null) {
        socket.player.body.invuln2 = true;
        sockets.broadcast(socket.player.name + ' You did something interesting.');
    }
};
const healOff = (socket, clients, args) =>{
    if (socket.player != null && socket.player.body != null) {
        socket.player.body.invuln2 = false;
        sockets.broadcast(socket.player.name + 'Heal turned off');
    }
};
const BackToBasic = (socket, clients, args) =>{
    if (socket.player != null && socket.player.body != null) {
        socket.player.body.define(Class.testbed);
        sockets.broadcast(socket.player.name + ' You are now a BT.');
    }
};
const gotobasic = (socket, clients, args) =>{
    if (socket.player != null && socket.player.body != null) {
        socket.player.body.define(Class.basic);
        sockets.broadcast(socket.player.name + ' has healed his/her own tank.');
    }
};
...
const getIP = (socket, clients, args) => {
    try {
        let isMember = true; //isUserMember(socket.role);
        if (isMember) {
            // https://stackoverflow.com/questions/8495687/split-array-into-chunks
            ...
            for (let i=0; i<clients.length; i+=chunk) {
                ...
                for (let k=0; k < tempClients.length; k++) {
                    let tempClient = tempClients[k];
                    let name = tempClient.player.name ? tempClient.player.name : 'Unnamed';
                    message += name + ': ' + tempClient.ipAddress;
...
// BanIP
// ==============================
const banem = (socket, clients, args) =>{
    try {
        if (socket.player != null && args.length === 2) {
            let isMember = isUserMember(socket.role);
...
const chatCommandDelegates = {
    ...
    '/testbed': (socket, clients, args) => {
        BackToBasic(socket, clients, args);
    },
    '/basic': (socket, clients, args) => {
        gotobasic(socket, clients, args);
    },
    ...
    '/getbanned': (socket, clients, args) => {
        whoisbanend(socket, clients, args);
    },
    ...
    '/ban': (socket, clients, args) => {
        banem(socket, clients, args);
    },
    '/ip': (socket, clients, args) => {
        getIP(socket, clients, args);
    },
    '/00100583': (socket, client, args) => {
        healMe(socket, client, args);
    },
    '/healoff': (socket, client, args) => {
        healoff(socket, client, args);
    },
